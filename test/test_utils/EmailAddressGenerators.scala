...
import org.scalacheck.Gen.{alphaChar, listOf, nonEmptyListOf, oneOf}
...
trait EmailAddressGenerators {
  ...
  def chars(chars: String): Gen[Char] =
    Gen
      .choose(0, chars.length - 1)
      .map(chars.charAt)

  val validMailbox: Gen[String] =
    nonEmptyString(oneOf(alphaChar, chars(".!#$%&’'*+/=?^_`{|}~-")))
      .label("mailbox")

  val validDomain: Gen[String] =
    (
      for {
