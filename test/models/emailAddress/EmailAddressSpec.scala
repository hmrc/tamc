...
import models.emailAddress.EmailAddress.{Domain, Mailbox}
...
class EmailAddressSpec
  ...
  "An EmailAddress class" should {
    ...
    "return false when .isValid is called with an invalid email address" in {
      EmailAddress.isValid("sausages") should be(false)
    }

    "return `not a valid email address` when js.validate fails" in {
      val exception = intercept[JsResultException](JsString("sausages").as[EmailAddress])

      exception.errors.head._2.head.message should be("not a valid email address")
    }
  }
